...
- Default voice: plain language (the primary user is a non-researcher — PM, designer, customer-success, founder). If `team-context/team-context.md` exists in the plugin directory, read it and apply its `# Voice` override.
...
Load `references/pre-flight-gate.md`. Run the user's input through the combined gate. If any of these conditions apply, **refuse-or-redirect**: surface the issue and recommend an alternative (skip the research, use analytics, do desk research, talk to the team, switch from AI-substitute to AI-augmented). Do not draft a brief.

Critical checks (full content in `references/pre-flight-gate.md`):
...
If the gate passes, proceed. If it fails, output the redirect and STOP.

### Step 2 — Why / What / Who framing

Before refining the question, establish the framing triad. Ask the user (or infer from their input):
...
If any of these is missing or vague, ask the user one targeted question to get it. Do not invent answers.

The "decision this research enables" is the sharpest framing the Field Guide names — if the user can't articulate a decision, return to the pre-flight gate (the "agency to act" check).

### Step 3 — Question refinement

Load `references/question-writing.md`. Run the user's research question through the universal rubric:
...
Given the refined question, Why/What/Who, and surfaced assumptions, identify which planning patterns apply. Load relevant pattern files from `references/patterns/`:

**Method patterns** (`references/patterns/methods/`) — apply based on the dominant method:
...
**Context patterns** (`references/patterns/contexts/`) — apply based on conditions:
...
Load `references/methodology-selection.md`. Run the multi-axis rubric:
...
**If the recommended method is interview-style** (1:1, focus group, generative interview, etc.), also run the AI-moderation-fit check from `references/patterns/methods/ai-moderation-fit.md`. Apply the 5-step rubric. Decide: AI moderation, human moderation, or hybrid. Document the choice in the brief.
...
Load `templates/default-brief.md` for the canonical 6-field structure: What / Why / How / Who / When and where / Next steps.
...
- **Who**: real humans (criteria, sourcing — must be named), sample size (from `references/sample-sizes.md`), audience for insights
- **When and where**: timeline, budget, tools, incentives
- **Next steps**: deliverables, activation plan, follow-up

Reference `references/sample-sizes.md` for the canonical N-by-method table + 11% no-show buffer.
...
Load `references/anti-patterns.md`. Scan the draft brief for:
...
If `team-context/team-context.md` has a `# Custom handoff` section, use that text instead of the default UI MCP language.

### Step 11 — Apply team-context overrides + return

If `team-context/team-context.md` exists, apply per-team overrides:
...
- Quote verbatim from references when applying frameworks (e.g., when refusing on the pre-flight gate, quote the specific reason).
...
- Draft a brief before completing the pre-flight gate.
...
If the user disagrees with your refusal at the pre-flight gate, quote the specific gate that failed and ask them to address that condition. If they want a brief anyway despite the gate, ask them to acknowledge they're proceeding despite the gate's recommendation, and document that acknowledgment in the brief.
